...
const buildReviewSubmittedHtml = ({ review, candidate, interviewer, adminUrl, appName }) => {
    ...
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="background:white;border-radius:10px;overflow:hidden">
        <tr><td style="padding:24px 32px;background:#0f172a;color:white">
          <div style="font-size:13px;opacity:.85;letter-spacing:.06em;text-transform:uppercase">Review Submitted</div>
          <div style="font-size:22px;margin-top:6px;font-weight:600">${escapeHtml(candidate.name)} — ${avg}/5</div>
        </td></tr>
        <tr><td style="padding:24px 32px;line-height:1.6">
          <p style="margin:0 0 14px"><strong>${escapeHtml(interviewer.name)}</strong> submitted a review for <strong>${escapeHtml(candidate.name)}</strong>.</p>
...
const buildReviewSubmittedText = ({ review, candidate, interviewer, adminUrl, appName }) => {
  const avg = fmtAvg(review.ratings);
  return `Review submitted by ${interviewer.name} for ${candidate.name}.
...
module.exports = { buildReviewSubmittedHtml, buildReviewSubmittedText };
