...
const presentCandidate = (candidate) => ({
  ...
  jobDescriptionId: candidate.jobDescriptionId || null,
...
const createCandidate = async ({ name, email, jobDescriptionId }, adminId) => {
  // The selected JD drives tech stack + experience. detail() throws 404 if missing.
  const jd = await jdService.detail(jobDescriptionId);
  const { token, expiresAt } = generateTestToken();
  const candidate = await candidateRepository.create({
    name,
    email,
    jobDescriptionId,
    techStack: jdService.tokenize(jd.techStack),
    experience: jd.experience,
    testToken: token,
    tokenExpiresAt: expiresAt,
    createdBy: adminId,
    // questionCount / durationMinutes keep model defaults until the Send-test step.
...
const runScreeningFor = async (candidate, { buffer } = {}) => {
  // Prefer the explicitly linked JD; fall back to skill-overlap matching for
  // candidates created before JD linking existed.
  let jd = null;
  if (candidate.jobDescriptionId) {
    jd = await jdService.detail(candidate.jobDescriptionId).catch((err) => {
      // 404 (JD deleted) is an expected fall-through to the matcher; log anything else.
      if (err?.statusCode !== 404) {
        logger.warn('Linked JD lookup failed during screening; falling back to matcher', {
  ...
  if (!jd) {
    jd = await jdService.findBestMatch(candidate.techStack, candidate.experience);
  }
  if (!jd) {
...
const sendTest = async (id, { questionCount, durationMinutes, techStack } = {}) => {
  const candidate = await candidateRepository.findById(id);
  if (!candidate) throw ApiError.notFound('Candidate not found');
  ...
  // Apply test-config overrides chosen at the Send-test step.
  if (Array.isArray(techStack) && techStack.length > 0) candidate.techStack = techStack;
  if (Number.isFinite(questionCount)) candidate.questionCount = questionCount;
  if (Number.isFinite(durationMinutes)) candidate.durationMinutes = durationMinutes;
