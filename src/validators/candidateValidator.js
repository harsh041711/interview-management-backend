...
const createCandidateSchema = {
  body: Joi.object({
    ...
    jobDescriptionId: objectId.required(),
  }),
};
...
const sendTestSchema = {
  ...
  body: Joi.object({
    ...
    techStack: Joi.array().items(Joi.string().min(1).max(60)).min(1).max(10).optional(),
  }),
};

module.exports = { createCandidateSchema, idParamSchema, listCandidatesSchema, rejectSchema, sendCodingTestSchema, sendTestSchema };
