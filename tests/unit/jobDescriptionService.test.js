...
describe('jobDescriptionService.pickBestMatch', () => {
  const mern = { id: 'jd-mern', techStack: 'react,node,mongodb,mysql', experience: 'mid' };
  const reactSr = { id: 'jd-react-sr', techStack: 'react', experience: 'senior' };
  const python = { id: 'jd-py', techStack: 'python,django', experience: 'mid' };

  test('matches when ANY candidate tag overlaps a comma-separated JD techStack', () => {
    const result = jdService.pickBestMatch([mern], ['MERN', 'Node', 'React'], 'senior');
    expect(result.id).toBe('jd-mern');
  });

  test('returns null when no skill overlaps', () => {
    const result = jdService.pickBestMatch([python], ['React', 'Node'], 'mid');
    expect(result).toBeNull();
  });

  test('prefers a JD whose experience matches over one that only overlaps skills', () => {
    const result = jdService.pickBestMatch([mern, reactSr], ['React'], 'senior');
    expect(result.id).toBe('jd-react-sr');
  });

  test('relaxes experience: still matches on skills when no experience-level JD exists', () => {
    const result = jdService.pickBestMatch([mern], ['React'], 'senior');
    expect(result.id).toBe('jd-mern');
  });

  test('breaks experience ties by greatest skill overlap', () => {
    const broad = { id: 'jd-broad', techStack: 'react,node', experience: 'mid' };
    const narrow = { id: 'jd-narrow', techStack: 'react', experience: 'mid' };
    const result = jdService.pickBestMatch([narrow, broad], ['React', 'Node'], 'mid');
    expect(result.id).toBe('jd-broad');
  });

  test('returns null for empty candidate stacks', () => {
    expect(jdService.pickBestMatch([mern], [], 'mid')).toBeNull();
    expect(jdService.pickBestMatch([mern], null, 'mid')).toBeNull();
  });
});

describe('jobDescriptionService.findBestMatch', () => {
  beforeEach(() => jest.clearAllMocks());

  test('pulls active JDs and returns the best skill match (relaxed experience)', async () => {
    jdRepo.listActive.mockResolvedValue([
      { id: 'jd-mern', techStack: 'react,node,mongodb,mysql', experience: 'mid' },
    ]);
    const result = await jdService.findBestMatch(['MERN', 'Node', 'React'], 'senior');
    expect(result.id).toBe('jd-mern');
  });

  test('returns null when there are no overlapping active JDs', async () => {
    jdRepo.listActive.mockResolvedValue([
      { id: 'jd-py', techStack: 'python,django', experience: 'mid' },
    ]);
    const result = await jdService.findBestMatch(['React'], 'senior');
    expect(result).toBeNull();
  });
});
